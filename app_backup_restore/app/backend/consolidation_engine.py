...
class ConsolidationEngine:
    ...
    def _detect_sequence_archetype(cls, req_text: str, req_id: str = "") -> Optional[Dict[str, Any]]:
        ...
        for arch in _SEQUENCE_ARCHETYPES:
            if arch["position"] == "Contract" and any(k in req_id_str for k in ["9170", "9171"]):
                return arch
            if arch["position"] == "Expand" and any(k in req_id_str for k in ["9177", "9895"]):
                return arch
            if arch["position"] == "VOID" and any(k in req_id_str for k in ["9209", "9210"]):
                return arch
            if any(m in text for m in arch["step_markers"]) and ("fault" in text or "range" in text or "ivt" in text):
                return arch
        ...
        return [
            {
                ...
                "description": (
                    ...
                    f"{fault} is False, {complete} = True is set, and IVT Response is "
                    f"transmitted on STL_Bus with IVT_Receipt_Envelope = 0x0017 and "
                    f"Harmonizing_Failed = False."
                ),
                "initial_condition": (
                ...
                "test_inputs": (
                    f"{avg} = {arch['in_range']} (within {pos} Harmonizing Range); "
                    f"LRU Configuration = LRU_1."
                ),
                "expected_result": (
                    f"{fault} = False; {complete} = True; IVT Response transmitted on "
                    f"STL_Bus with IVT_Receipt_Envelope = 0x0017, IVT_Receipt_Content: "
                    f"Harmonizing_Failed = False, {complete} = True."
                ),
            ...
            },
            {
                ...
                "description": (
                    ...
                    f"{fault} is True, and IVT Response is transmitted on STL_Bus with "
                    f"IVT_Receipt_Envelope = 0x0017 and Harmonizing_Failed = True."
                ),
                "initial_condition": (
                    f"Operational Mode: FLIGHT_MODE; Target Asset Type: ATYPE_1; "
                    f"Target LRU: LRU_1; Commanded to {pos} Position; "
                ...
                "test_inputs": (
                    f"{avg} = {arch['out_range']} (outside {pos} Harmonizing Range); "
                    f"LRU Configuration = LRU_1."
                ),
                "expected_result": (
                    f"{fault} = True; IVT Response transmitted on STL_Bus with "
                    f"IVT_Receipt_Envelope = 0x0017, IVT_Receipt_Content: "
                    f"Harmonizing_Failed = True."
                ),
                "pass_criteria": "Observed software behaviour matches expected_result.",
                "related_requirements": "",
                "test_procedure_notes": (
    ...
    def consolidate_multi_member_records(cls, tcs: List[Dict[str, Any]], req_id: str,
        ...
        members = cls._extract_psr_members(req_text)
        if len(members) < 3:
            members = [
                "Harmonizing_Active", "Status", "Phase", "Progress",
                "Subphase", "Subphase_Progress", "Step", "Step_Progress",
                "Act_Disp_Raw_Avg",
            ]
        ...
        return [
            {
                ...
                "test_type": "NORMAL",
                "description": (
                    f"Verify that upon entry to Harmonizing Mode, all {len(members)} "
                    f"PSR Harmonizing data members ({members_str}) are properly published "
                    f"on STL_Bus in accordance with the Harmonizing PSR specification."
                ),
                "initial_condition": (
                ...
                "expected_result": (
                    f"All {len(members)} PSR data members ({members_str}) are actively "
                    f"published on STL_Bus with valid default values and status flags "
                    f"matching specification."
                ),
                "pass_criteria": (
                    f"Observed STL_Bus values match expected result for all "
                    f"{len(members)} data members."
                ),
                "related_requirements": "",
                "test_procedure_notes": (
                    "Verification mechanism defined by verification environment."
                ),
            }
        ]
    ...
    def deduplicate_test_cases(cls, tcs: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        ...
        Preserves distinct branches (NORMAL vs DC vs BOUNDARY vs ROBUSTNESS).
        """

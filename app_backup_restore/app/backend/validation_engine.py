...
class ValidationEngine:
    ...
    def validate_and_sanitize(cls, test_cases: List[Dict[str, Any]],
        ...
        for idx, tc in enumerate(test_cases, 1):
            ...
            if "ivt_mode_modelgc" in init_cond.lower():
                ...
                init_cond = re.sub(
                    r';?\s*IVT_Mode_ModeLgc\s*=\s*(True|False)', '',
        ...
        if report.atype_coverage["ATYPE_2"] == 0 or report.atype_coverage["ATYPE_3"] == 0:
            report.all_passed = False
            report.details.append("WARNING: Missing ATYPE_2 or ATYPE_3 coverage in test suite.")

        if report.lru_coverage["LRU_5"] == 0:
            report.all_passed = False
            report.details.append("WARNING: Missing LRU_5 coverage in test suite.")
    ...
    def audit_test_cases(cls, test_cases: List[Dict[str, Any]],
        ...
        for at in ("ATYPE_2", "ATYPE_3"):
            if atype_hits[at] == 0:
                checks["applicability"]["passed"] = False
                checks["applicability"]["issues"].append(f"No test case references {at}.")
        if lru_hits["LRU_5"] == 0:
            checks["applicability"]["passed"] = False
            checks["applicability"]["issues"].append("No test case references LRU_5.")
